...
class Estimator(object):
	...
	#### ----- YOUR CODE GOES HERE ----- ####
		t = self.step_size
		trans = sens.vel_trans
		theta = sens.vel_ang
				
		x_pred = self.x[0] + t * trans * math.cos(self.x[2])
		y_pred = self.x[1] + t * trans * math.sin(self.x[2])
		theta_pred = self.x[2] +t * theta
		...
		F = [[1,0,((-t)*trans*math.sin(self.x[2]))], [0, 1, (t*trans*math.cos(self.x[2]))], [0,0,1]]
