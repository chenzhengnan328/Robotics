...
class CCIK(object):
    ...
    def get_deltax(self, b_T_eed, b_T_eec, joint_transforms):
        ...
        Deltax = numpy.concatenate((deltax, deltar), axis = None)
    ...
    def get_cartesian_command(self, command):
        ...
        Deltax = self.get_deltax(b_T_eed, b_T_eec, joint_transforms)
        ee_V_ee = Deltax 
    ...
    def get_jacobian(self, b_T_eec, joint_transforms):
        ...
        for j in range(self.num_joints):
            ...
            #Vj.append(Sj_T_eec)
            #Vj = np.array(Vj)
            Vj = numpy.concatenate((Sj_T_eec, eec_rotation_j), axis = 0 )
            rotation_axis = numpy.transpose(self.joint_axes[j])
            Jj = numpy.dot(Vj, rotation_axis)
    ...
    def get_ik_command(self, command):
        ...
        for _ in range(3):
            ...
            for _ in range(self.num_joints):
                q_current.append(random.random()* math.pi*2)    
            ...
            while time.time() - start_point <= 10:
                ...
                Deltax = self.get_deltax(b_T_eed, b_T_eec, joint_transforms)
                ee_V_ee = Deltax
                if all(abs(distance) <= 0.001 for distance in Deltax):
                    marker = True
                    break
                J = self.get_jacobian(b_T_eec, joint_transforms)
                Jplus= numpy.linalg.pinv(J, 0.01)
                delta_q = numpy.dot(Jplus, ee_V_ee)
                q_current += delta_q           
