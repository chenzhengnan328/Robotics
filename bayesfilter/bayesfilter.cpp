...
int main() {  
  ...
  for (int t = 0; t < time_steps; ++t) {
    ...
    for (unsigned int i = 0; i < map_size; ++i) {
        ...
        float motion = motion_model(pseudo_position, movement_per_timestep, priors, map_size, control_stdev);
        vector<float> pseudo_ranges = pseudo_range_estimator(landmark_positions, pseudo_position);
        float obs_prob = observation_model(landmark_positions, observations, pseudo_ranges, distance_max, observation_stdev);
        posteriors[i] = motion*obs_prob;
    }
